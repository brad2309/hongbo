...
public class MainServlet extends HttpServlet{
	...
	private Map<String,Method> uriMethodMap = new HashMap<String, Method>();//路径对应Method
	private Map<String,Object> uriInstanceMap = new HashMap<String, Object>();//路径对应类的实例
	private Map<Method,String[]> methodParameterNameMap = new HashMap<Method, String[]>();//路径的Method对应其参数名
	...
	private void initUriMap() throws Exception{
		List<Class<?>> actionClassList = ClassUtil.getClasses("com.ttpai.mvc", Action.class);
		if(actionClassList==null){
			return;
		}
		for(Class<?> actionClass:actionClassList){
			String prefix = actionClass.getSimpleName();
			if(prefix.length()<7||!prefix.endsWith("Action")){
				throw new RuntimeException("ileegal action class name:"+prefix);
			}
			prefix = (char)(prefix.charAt(0)+32)+prefix.substring(1,prefix.length()-6);
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(returnType.equals(String.class)){
			...
			if(StringUtils.isNoneBlank(resultStr)&&resultStr.charAt(0)=='@'){//直接返回
	...
	private Object setParameters(Class<?> c,HttpServletRequest req) throws Exception{
		...
		while(enums.hasMoreElements()){//遍历请求参数
			...
			if(StringUtils.isBlank(value)){//参数值为空不处理
				continue;
			}
